...
public class Perusahaan {
    ...
    public void tambahPegawaiKeCabang(String namaCabang, String namaPegawai, String jabatan, int gaji, boolean isPegawaiTetap, int lamaBekerja){
        for (int i = 0; i < daftarCabang.length; i++) {
            if (daftarCabang[i].getNama().equalsIgnoreCase(namaCabang)){
                daftarCabang[i].tambahPegawai(namaPegawai, jabatan, gaji, isPegawaiTetap, lamaBekerja);
                break;
            }
        }
    }
